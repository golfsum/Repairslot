...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{
  ...
  if(industry){
    if(slug==='appliance-repair-scheduling-software'){
      const title='Appliance Repair Scheduling Software: Checklist & Demo';
...
function Header(){return <header className="nav shell"><a className="brand" href="/"><span className="brandMark"><Wrench size={18}/></span><span>RepairSlot</span></a><nav><a href="/resources">Resources</a><a href="/#pricing">Pricing</a><a className="button buttonSmall" href="/book/demo">Sample demo</a></nav></header>}
...
function ResourcePageView({slug,page}:{slug:string;page:(typeof resourcePages)[number]}){
  const isInstallGuide=slug==='how-to-add-online-booking-to-repair-website';
  ...
  const faqSchema={'@context':'https://schema.org','@type':'FAQPage',mainEntity:faq.map(x=>({'@type':'Question',name:x.q,acceptedAnswer:{'@type':'Answer',text:x.a}}))};
  return <main className="seoPage"><Header/>
    <section className="seoHero shell"><div className="eyebrow">{page.eyebrow}</div><h1>{page.title}</h1><p>{page.intro}</p><div className="heroActions"><a className="button" href="/book/demo">Explore the sample booking demo</a><a className="ghostButton" href="/resources">Browse resources</a></div></section>
    <section className="section shell seoTwoCol"><div><div className="eyebrow">What to focus on</div><h2>Reduce friction between “I need this fixed” and “you’re booked.”</h2><p>Repair customers often arrive with high intent. A strong booking workflow keeps the next step obvious while still protecting the business from impossible appointment promises.</p><ul className="seoChecklist">{page.points.map(x=><li key={x}><Check/>{x}</li>)}</ul>{isInstallGuide?<p>Planning an appliance-repair flow? Use the <a href="/appliance-repair-scheduling-software">appliance repair scheduling software checklist and sample demo</a> to review intake, service-area and capacity checks. The sample does not create a real appointment.</p>:null}</div><div className="seoExampleCard"><span>RepairSlot approach</span><h3>Customer journey</h3><div className="seoExample">1. Choose the repair or service</div><div className="seoExample">2. Provide job and location details</div><div className="seoExample">3. See valid service windows</div><div className="seoExample">4. Confirm the booking</div><div className="seoWindow"><strong>Goal</strong><span>Less waiting, fewer lost leads</span></div></div></section>
    ...
    <Related current={slug}/><section className="cta"><div className="shell ctaInner"><div><div className="eyebrow light">Make the next step bookable.</div><h2>Turn more repair demand into scheduled work.</h2></div><a className="button lightButton" href="/book/demo">Explore the sample demo</a></div></section><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(faqSchema)}}/></main>
}
