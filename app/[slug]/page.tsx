...
function IndustryPage({slug,industry}:{slug:string;industry:(typeof industries)[number]}){
  ...
  return <main className="seoPage"><Header/>
    <section className="seoHero shell"><div className="eyebrow">Built for {industry.audience}</div><h1>{industry.name}</h1><p>{industry.intro}</p><div className="heroActions"><a className="button" href="/book/demo">Try the live booking demo</a><a className="ghostButton" href="/#pricing">See pricing</a></div></section>
    ...
    <section className="section shell seoTwoCol"><div><div className="eyebrow">Why generic calendars fall short</div><h2>The booking should understand the repair before it offers a time.</h2><p>{industry.audience.charAt(0).toUpperCase()+industry.audience.slice(1)} need more than an empty appointment slot. The service type, location, expected duration and technician capacity all affect what can actually be booked.</p><ul className="seoChecklist">{industry.challenges.map(x=><li key={x}><Check/>{x}</li>)}</ul><h3 className="seoSubhead">Useful intake to collect first</h3><ul className="seoChecklist">{industry.questions.map(x=><li key={x}><Check/>{x}</li>)}</ul></div><div className="seoExampleCard"><span>Example booking options</span><h3>What needs service?</h3>{industry.examples.map(x=><div className="seoExample" key={x}>{x}</div>)}<div className="seoWindow"><strong>Next available</strong><span>Tomorrow · 10 AM–12 PM</span></div></div></section>
