...
import {competitorPages} from '../../lib/competitor-seo';
...
const featuredResources=[
 ...
 {name:'Jobber Alternative for Repair Business Online Booking',url:'https://repairslot.com/compare/jobber-alternative-for-online-booking'},
...
export default function Resources(){
 return <main className="seoPage">
  ...
  <section className="seoHero shell"><div className="eyebrow">RepairSlot resource library</div><h1>Repair business booking, scheduling and conversion resources</h1><p>Browse repair-industry pages, practical booking guides, customer-conversion features, software comparisons and free calculators.</p></section>
  ...
  <section className="section shell"><div className="sectionIntro narrow"><div className="eyebrow">Software comparisons</div><h2>RepairSlot alternatives and booking comparisons</h2><p>For repair businesses deciding whether they need a full field-service suite or a focused customer-facing booking layer.</p></div><div className="resourceGrid">{competitorPages.map(x=><a href={`/compare/${x.slug}`} className="resourceCard" key={x.slug}><BookOpen/><div><strong>{x.title}</strong><span>{x.bestFor}</span></div><ArrowRight/></a>)}</div></section>
  <section className="section industrySection"><div className="shell"><div className="sectionIntro narrow"><div className="eyebrow">Features and problems</div><h2>Fix booking friction</h2></div><div className="resourceGrid">{group('feature').map(x=><a href={`/${x.slug}`} className="resourceCard" key={x.slug}><BookOpen/><div><strong>{x.title}</strong><span>{x.intro}</span></div><ArrowRight/></a>)}</div></div></section>
  <section className="section shell"><div className="sectionIntro narrow"><div className="eyebrow">Guides</div><h2>Practical repair-business playbooks</h2></div><div className="resourceGrid">{group('guide').map(x=><a href={`/${x.slug}`} className="resourceCard" key={x.slug}><BookOpen/><div><strong>{x.title}</strong><span>{x.intro}</span></div><ArrowRight/></a>)}</div></section>
  <section className="section industrySection"><div className="shell"><div className="sectionIntro narrow"><div className="eyebrow">Decision pages</div><h2>Compare booking approaches</h2></div><div className="resourceGrid">{group('comparison').map(x=><a href={`/${x.slug}`} className="resourceCard" key={x.slug}><BookOpen/><div><strong>{x.title}</strong><span>{x.intro}</span></div><ArrowRight/></a>)}</div></div></section>
  <section className="section shell"><div className="sectionIntro narrow"><div className="eyebrow">Free calculators</div><h2>Put numbers behind the booking problem</h2></div><div className="resourceGrid"><a href="/tools/missed-call-revenue-calculator" className="resourceCard"><CalcIcon/><div><strong>Missed Call Revenue Calculator</strong><span>Estimate monthly repair revenue represented by missed calls.</span></div><ArrowRight/></a>{tools.map(x=><a href={`/tools/${x.slug}`} className="resourceCard" key={x.slug}><CalcIcon/><div><strong>{x.title}</strong><span>{x.description}</span></div><ArrowRight/></a>)}</div></section>
